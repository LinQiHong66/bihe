...
import com.inesv.digiccy.back.utils.GoogleUpdateFileUtil;
import com.inesv.digiccy.back.utils.HttpUtil;
...
public class CoinController {
	...
	@ResponseBody
	public Map<String, Object> update(MultipartFile picture , HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		Map<String, Object> map = new HashMap<>();
	    String res = GoogleUpdateFileUtil.uploadFile(picture,"btc666");
		map.put("res", res);
	...
	@ResponseBody
	public Map<String, Object> addCoin(@RequestParam String no, @RequestParam String name, @RequestParam String code,
			@RequestParam String address, @RequestParam String host, @RequestParam String post,
			@RequestParam String wallet_name, @RequestParam String wallet_pwd, @RequestParam String wallet_lockpwd,
			@RequestParam String buy_poundatge, @RequestParam String sell_poundatge, @RequestParam String block,
			@RequestParam String sell_withdraw_poundatge_one, @RequestParam String sell_withdraw_poundatge_twe,
			@RequestParam String sell_withdraw_poundatge_three) {
		Map<String, Object> coinAndWalletLinkMap = coinValidata.addCoin(Integer.valueOf(no), name, code, address, host,
				post, wallet_name, wallet_pwd, wallet_lockpwd, new BigDecimal(buy_poundatge),
				new BigDecimal(sell_poundatge), Integer.valueOf(block),  new BigDecimal(sell_withdraw_poundatge_one),
				 new BigDecimal(sell_withdraw_poundatge_twe),  new BigDecimal(sell_withdraw_poundatge_three));
		return coinAndWalletLinkMap;
	}

	@RequestMapping(value = "updateCoin", method = RequestMethod.POST)
	@ResponseBody
	public Map<String, Object> updateCoin(@RequestParam String no, @RequestParam String name, @RequestParam String code,
			@RequestParam String address, @RequestParam String host, @RequestParam String post,
			@RequestParam String wallet_name, @RequestParam String wallet_pwd, @RequestParam String wallet_lockpwd,
			@RequestParam String buy_poundatge, @RequestParam String sell_poundatge, @RequestParam String block,
			@RequestParam String sell_withdraw_poundatge_one, @RequestParam String sell_withdraw_poundatge_twe,
			@RequestParam String sell_withdraw_poundatge_three) {
		Map<String, Object> coinMap = coinValidata.updateCoin(Integer.valueOf(no), name, code, address, host, post,
				wallet_name, wallet_pwd, wallet_lockpwd, new BigDecimal(buy_poundatge), new BigDecimal(sell_poundatge),
				Integer.valueOf(block),  new BigDecimal(sell_withdraw_poundatge_one),
				 new BigDecimal(sell_withdraw_poundatge_twe),  new BigDecimal(sell_withdraw_poundatge_three));
    ...
    public ModelAndView uploadIcon(@RequestParam String id,@RequestParam MultipartFile icon,
                                   HttpServletResponse rsp, HttpServletRequest request) throws IOException {
       //签名
            ...
            if (file != null) {
                ...
                try {
                    ...
                    if(img.length()<512000){
                	   ...
                	   try {
                			  //QiniuUploadUtil.upLoadImage(icon.getInputStream(),fileName[0]);  //七牛上传图片
                		   savePath =  GoogleUpdateFileUtil.uploadFile(icon, "btc666");
