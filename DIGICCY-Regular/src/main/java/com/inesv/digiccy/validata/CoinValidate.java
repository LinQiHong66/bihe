...
public Map<String, Object> getInfoByNo(int coinNo){
    ...
    public Map<String,Object> validateBalanceInfo(Integer userNo){
        ...
        //人民币资产
        List<UserBalanceDto> rmbBalances = querycoin.queryUserBalance(userNo,0); //0人民币
        List<CoinDto> rmbCoins =  querycoin.queryCoinTypeByCoinNo(0);
    	CoinDto rmbCoin=rmbCoins.get(0);
    	rmbBalances.get(0).setCoinName(rmbCoin.getCoin_name());
    	rmbBalances.get(0).setCoinCode(rmbCoin.getCoin_core());
    	rmbBalances.get(0).setCurPrice(1);
    	rmbBalances.get(0).setSumCurPrice(rmbBalances.get(0).getTotal_price().doubleValue());
