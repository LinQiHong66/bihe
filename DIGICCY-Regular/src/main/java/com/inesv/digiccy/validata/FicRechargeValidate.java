...
//    }
   ...
   /* public void validateRechargeCoinNew() throws SQLException{
    ...
    public void validateRechargeCoinNew() throws SQLException{
        ...
        for(CoinDto coin : coinList){
            ...
            if(coin.getCoin_no()==20 || coin.getCoin_no()==40){ //以太坊|以太经典
            	...
            	for(String account : list){   //遍歷所有賬戶的充值信息，添加到充值表
      	        	System.err.println("=========account 进行充值处理===:"+account);
      	        	List<Map<String, Object>> translist =new ArrayList<Map<String, Object>>();
      	         try{
      	        	translist = this.getEthRecharge(account);  //获取账户的充值信息
      	         }catch(Exception e)
      	         {
      	        	 System.err.println("============获取以太坊充值信息异常================");
      	        	 e.printStackTrace();
      	         }
      	        	for(Map<String, Object> m: translist){
      	        		String txid = m.get("hash").toString();
      	        		String amount = m.get("value").toString();
      	        		BigDecimal amountBig=new BigDecimal(amount);
      	        		BigDecimal wei=new BigDecimal("1000000000000000000");
      	        		BigDecimal bigPrice = amountBig.divide(wei);
      	        		String to_address= m.get("to").toString();  //充值賬戶
      	        		String confirmations= m.get("confirmations").toString();  //确认数
      	        		...
      	        		if(rechargeInfo==null){   //没有充值记录，插入一条
                             ...
                             //虚拟币充值
                              ficRechargePersistence.ficRechargeTransac(userNo, coinNo, to_address, bigPrice, RechargeState.RECHARGED.getValue(), txid, nowEnble, nowTotalprice);
            ...
            if(!"none".equals(transJson)){
                ...
                for(Map<String,Object> trans: transList){
                    ...
                    if (userInfo == null) continue;
                    logger.info("All check ok");
                    String to_ddress = (String) trans.get("address");
                    int userNo = userInfo.getUser_no();
                    String txid = (String) trans.get("txid");
                    Double price = transactionResult.getResultCoin(trans.get("amount"));
                    BigDecimal bigPrice = BigDecimal.valueOf(price);
                    ...
                    Integer confirmations = trans.get("confirmations") == null ? 0 : (Integer) trans.get("confirmations");
                    if(rechargeInfo == null){  //没有充值记录，则插入一个
                        if(confirmations >= 6){ //没有充值记录，则插入一个
                            ...
                            //虚拟币充值
                            ficRechargePersistence.ficRechargeTransac(userNo, coinNo, to_ddress, bigPrice, RechargeState.RECHARGED.getValue(), txid, nowEnble, nowTotalprice);
                        } 
                    } 
                   }
                logger.info("Recharge Mission Finish!");
    ...
    /*
     * 获取以太坊的充值记录（调用以太坊浏览器APi）
     * **/
    public static List<Map<String,Object>> getEthRecharge(String account) {
 	
    	HttpRequest request = HttpRequest.get("http://api.etherscan.io/api?module=account&action=txlist&address=" + account + "&startblock=0&sort=desc&apikey=K2ERUX1KWMJVFU4KN3XKV18TUXCKE6CB2A");
		request.contentType("User-Agent", "Mozilla/4.0 (compatible; MSIE 5.0; Windows NT; DigExt)");
	    HttpResponse response = request.send();
		String respJson = response.bodyText();
		Map<String, Object> respMap = JSON.parseObject(respJson, Map.class);
		List<Map<String,Object>> dataList = (List<Map<String, Object>>) respMap.get("result");
		//只获取充值交易
		for(Map<String,Object> map : dataList) {
			if(!map.get("to").equals(account)) { 
				dataList.remove(map);
			}
		}
    	return dataList;
    ...
    public static void main(String[] args) {
    		...
    		String account = "0x9214fb444273be2065d33169711ebee3e2ea675d";
        	List<Map<String,Object>> arg0 = FicRechargeValidate.getEthRecharge(account);
